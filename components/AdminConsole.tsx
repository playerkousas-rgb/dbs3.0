...
/**
 * 秘書後台主控台（AdminConsole）
 *
 * 由兩個地方共用：
 *  1. /admin — 區秘書以該區 STAFF_TOKEN 登入（token 由輸入而來）
 *  2. /super — 平台超管全域模式（token 傳入空字串；
 *              權限改由 /api/super/proxy 從 Vercel 環境變數注入）
 */
...
export function AdminConsole({ token, onLogout, superMode = false }: AdminConsoleProps) {
  ...
  // 載入概覽數字（超管模式 = 免密鑰，權限由伺服器注入）
  useEffect(() => {
    let alive = true;
    api.adminGetDashboard(token)
      .then((res: any) => { if (alive && res?.success) setDashboard(res.stats || {}); })
      .catch(() => { /* 登入時已驗證過，這裡失敗只影響統計數字 */ });
    return () => { alive = false; };
  ...
  const openPrint = (certId: string) => {
    const path = `/admin/print-cert/${encodeURIComponent(certId)}`;
    const url = withDistrict(superMode ? path : `${path}?token=${encodeURIComponent(token)}`);
  ...
  return (
    <div>
      <div style={{ background: 'white', padding: '20px', borderRadius: '12px', marginBottom: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ color: '#003366', margin: 0 }}>📊 秘書控制台{superMode ? '（超管全域模式）' : ''}</h2>
        <button onClick={onLogout} style={{ padding: '8px 16px', borderRadius: '6px', border: '1px solid #ddd', background: 'white', cursor: 'pointer' }}>
          {superMode ? '登出超管' : '登出'}
        </button>
      </div>

      {superMode && (
        <div style={{ background: '#e8f5e9', border: '1px solid #a5d6a7', color: '#1b5e20', padding: '12px', borderRadius: '8px', marginBottom: '16px', fontSize: '14px' }}>
          🌐 超管全域模式：權限由 Vercel 環境變數（SUPER_KEY / DBS_區碼_STAFF_KEY）於伺服器端注入，畫面不會接觸任何區密鑰。
        </div>
      )}

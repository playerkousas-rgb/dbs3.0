...
function initializeSheets(ss) {
  ...
  var sheetsConfig = [
    ...
    },
    {
      ...
      data: [
        ['DISTRICT_CODE', CONFIG.DEFAULT_DISTRICT_CODE, '【必填】區碼（建議 3-4 個英文字，例如 ABC / CHW）'],
...
function getReadmeSheetData_() {
  return [
    ...
    ['主考指派模式', 'Config 的 EXAMINER_ASSIGNMENT_MODE 可由秘書後台一鍵切換：GROUP_PRIORITY=同旅 G 旅團主考優先；DISTRICT_PRIORITY=只用 D 區主考；NO_SAME_GROUP=所有合資格主考隨機但排除同旅團。'],
...
function doPost(e) {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'adminGetSettings': result = apiAdminGetSettings(data); break;
      case 'adminSetAssignmentMode': result = apiAdminSetAssignmentMode(data); break;
...
function apiDistrictApprove(data) {
  ...
  var groupId = getAppValue(appData, headers, 'group_id');
  var assignmentMode = getExaminerAssignmentMode_();
  var now = new Date(), nowStr = now.toISOString();
  ...
  if (examArrType === 'SELF_APPLY' && selfExaminerName && assignmentMode !== 'NO_SAME_GROUP') {
    var selfPick = findSelfSelectedExaminer_(selfExaminerName, badgeName, groupId, now);
    if (!selfPick.found) return { success: false, error: '自行安排的主考不在授權名單內', requiresManualOverride: true };
    if (!selfPick.valid) return { success: false, error: selfPick.error || '自行安排的主考不符合此申請資格', requiresManualOverride: true };

    // GROUP_PRIORITY：可採用自選同旅 G 或 D；DISTRICT_PRIORITY：只採用 D（即使 D 與考生同旅團也可）。
    if (assignmentMode === 'GROUP_PRIORITY' || (assignmentMode === 'DISTRICT_PRIORITY' && selfPick.scope === 'D')) {
      assignedExaminerId = selfPick.examinerId;
      assignedExaminerName = selfPick.name;
      assignedExaminerEmail = selfPick.email;
      assignedExaminerPhone = selfPick.phone;
    }
  }
  if (data.overrideExaminerId && !assignedExaminerId) {
    ...
    if (overrideEx) {
      var overrideHasD = overrideEx.districtBadges.indexOf(badgeName) >= 0;
      var overrideHasSameG = overrideEx.groupBadges.indexOf(badgeName) >= 0 && isSameGroupUnit_(overrideEx.unit, groupId);
      if (assignmentMode === 'DISTRICT_PRIORITY' && !overrideHasD) {
        return { success: false, error: '只用區主考模式下只可指派 D 區主考' };
      }
      if (assignmentMode === 'NO_SAME_GROUP') {
        var overrideSameUnit = isSameGroupUnit_(overrideEx.unit, groupId);
        if (overrideSameUnit) return { success: false, error: '同旅團不能擔任主考模式下，不可指派與考生同旅團的主考' };
        if (!overrideHasD && overrideEx.groupBadges.indexOf(badgeName) < 0) return { success: false, error: '改派主考無此章資格' };
      }
      if (assignmentMode === 'GROUP_PRIORITY' && !overrideHasD && !overrideHasSameG) {
        return { success: false, error: '改派主考不符合此章或旅團資格' };
      }
  ...
  if (!assignedExaminerId) {
    var autoAssign = autoAssignExaminer(badgeName, '', groupId);
...
function apiExaminerDecline(data) {
  ...
  var groupId = getAppValue(allData[rowIndex], headers, 'group_id');
  var newAssign = autoAssignExaminer(badgeName, oldExaminerId, groupId);
  if (!newAssign) {
...
function apiAdminGetPendingApplications(data) {
  ...
  return { success: true, applications: pending, availableExaminers: examinersList, assignmentMode: getExaminerAssignmentMode_(), assignmentModeLabel: getExaminerAssignmentModeLabel_() };
}
...
function _previewExaminerForApp_(examArrType, selfExaminerName, badgeName, groupId, examinersList) {
  var assignmentMode = getExaminerAssignmentMode_();
  var assignmentModeLabel = getExaminerAssignmentModeLabel_();

  // 不需派主考的安排
  if (examArrType === 'CERTIFICATE_EXCHANGE' || examArrType === 'APPROVED_COURSE' || examArrType === 'EXAM_DAY') {
    return { mode: 'no_examiner', label: '不需派主考', detail: CONFIG.EXAM_ARRANGEMENT[examArrType] || examArrType, valid: true, assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
  }

  // 自選主考：GROUP_PRIORITY 可採用同旅 G / D；DISTRICT_PRIORITY 只採用 D；NO_SAME_GROUP 不採用自選，改為隨機派非同旅。
  if (examArrType === 'SELF_APPLY' && selfExaminerName && assignmentMode !== 'NO_SAME_GROUP') {
    var found = null;
    ...
    if (!found) {
      return { mode: 'self', label: '⚠️ ' + selfExaminerName, detail: '不在主考名單，建議改派', valid: false, severity: 'warn', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
    }

    var isD = found.districtBadges.indexOf(badgeName) >= 0;
    var isG = found.groupBadges.indexOf(badgeName) >= 0;
    if (!isD && !isG) {
      return { mode: 'self', label: '❌ ' + selfExaminerName, detail: '無此章資格', valid: false, severity: 'error', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
    }
    if (found.currentLoad >= found.maxLoad) {
      return { mode: 'self', label: '⚠️ ' + selfExaminerName, detail: '已滿載 ' + found.currentLoad + '/' + found.maxLoad, valid: false, severity: 'warn', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
    }

    if (isD) {
      return { mode: 'self', label: '✅ ' + selfExaminerName, detail: '自選 D 區主考・' + assignmentModeLabel + '・負荷 ' + found.currentLoad + '/' + found.maxLoad, valid: true, examinerId: found.examinerId, scope: 'D', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
    }

    var sameUnit = isSameGroupUnit_(found.unit, groupId);
    if (isG && !sameUnit) {
      return { mode: 'self', label: '⚠️ ' + selfExaminerName, detail: 'G 主考但不同旅 (' + found.unit + ')', valid: false, severity: 'warn', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
    }
    if (isG && assignmentMode === 'GROUP_PRIORITY') {
      return { mode: 'self', label: '✅ ' + selfExaminerName, detail: '自選同旅 G 旅團主考・' + assignmentModeLabel + '・負荷 ' + found.currentLoad + '/' + found.maxLoad, valid: true, examinerId: found.examinerId, scope: 'G', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
    }
    // DISTRICT_PRIORITY：不採用自選 G，繼續用下方自動指派預覽（只找 D 區主考）。
    // NO_SAME_GROUP：不採用自選主考，繼續用下方自動指派預覽（隨機派非同旅）。
  }

  var best = pickBestExaminerCandidate_(badgeName, groupId, examinersList, null, assignmentMode);
  if (!best) {
    return { mode: 'auto', label: '❌ 無合資格主考', detail: (assignmentMode === 'DISTRICT_PRIORITY' ? '請先委任 D 區主考或切回旅團優先模式' : assignmentMode === 'NO_SAME_GROUP' ? '沒有非同旅團的合資格主考' : '請先委任主考或改用人手指派') + '・' + assignmentModeLabel, valid: false, severity: 'error', assignmentMode: assignmentMode, assignmentModeLabel: assignmentModeLabel };
  }

  var ignoredSelfNote = (examArrType === 'SELF_APPLY' && selfExaminerName && assignmentMode === 'DISTRICT_PRIORITY') ? '・自選 G 不採用' : (examArrType === 'SELF_APPLY' && selfExaminerName && assignmentMode === 'NO_SAME_GROUP') ? '・自選主考不採用，隨機派非同旅' : '';
  return {
    mode: 'auto',
    label: '🤖 ' + best.ex.name + ' (' + best.scope + ')',
    detail: assignmentModeLabel + ignoredSelfNote + '・負荷 ' + best.ex.currentLoad + '/' + best.ex.maxLoad + '・共 ' + best.totalCandidates + ' 位合資格',
    valid: true,
...
function apiAdminGetSettings(data) {
  ...
  return {
    ...
    assignmentModeLabel: getExaminerAssignmentModeLabel_(),
    assignmentModeOptions: [
      { value: 'GROUP_PRIORITY', label: '旅團主考優先', description: '同旅 G 旅團主考優先；沒有合適 G 時才派 D 區主考。' },
      { value: 'DISTRICT_PRIORITY', label: '只用區主考（公平模式）', description: '只會自動派 D 區主考；即使 D 主考本身是同旅團領袖，仍視為 ADC 已授權的區主考。' },
      { value: 'NO_SAME_GROUP', label: '同旅團不能擔任主考', description: '所有具該章資格的主考隨機分配，但排除與考生同旅團的主考。' }
    ]
  };
}
...
function apiAdminSetAssignmentMode(data) {
  ...
  return { success: true, assignmentMode: mode, assignmentModeLabel: getExaminerAssignmentModeLabel_(mode), message: '主考指派模式已更新為：' + getExaminerAssignmentModeLabel_(mode) };
}
...
// M. 主考工具函數
function autoAssignExaminer(badgeFullTitle, excludeExaminerId, groupId) {
  var examinersList = _getActiveExaminersForAdmin_();
  var best = pickBestExaminerCandidate_(badgeFullTitle, groupId, examinersList, excludeExaminerId || null, getExaminerAssignmentMode_());
  if (!best) return null;
  return {
    examinerId: best.ex.examinerId,
    name: best.ex.name,
    email: best.ex.email,
    phone: best.ex.phone,
    load: best.ex.currentLoad,
    scope: best.scope,
    assignmentMode: getExaminerAssignmentMode_()
  };
}

function pickBestExaminerCandidate_(badgeFullTitle, groupId, examinersList, excludeExaminerId, assignmentMode) {
  assignmentMode = normalizeExaminerAssignmentMode_(assignmentMode || getExaminerAssignmentMode_());
  var candidates = [];
  for (var i = 0; i < examinersList.length; i++) {
    var ex = examinersList[i];
    if (!ex || ex.examinerId === excludeExaminerId) continue;
    if (ex.currentLoad >= ex.maxLoad) continue;

    var hasD = ex.districtBadges.indexOf(badgeFullTitle) >= 0;
    var hasG = ex.groupBadges.indexOf(badgeFullTitle) >= 0;
    var unitSameGroup = isSameGroupUnit_(ex.unit, groupId);
    var sameGroupG = hasG && unitSameGroup;
    var priority = 99;
    var scope = '';

    if (assignmentMode === 'DISTRICT_PRIORITY') {
      // 只用區主考：D 主考可獲派，即使其本身是同旅團領袖也可。
      if (hasD) { priority = 0; scope = 'D'; }
    } else if (assignmentMode === 'NO_SAME_GROUP') {
      // 同旅團不能擔任主考：所有具該章資格的 D/G 主考均可進入候選，但同旅團一律排除。
      if (unitSameGroup) continue;
      if (hasD || hasG) { priority = 0; scope = hasD ? 'D' : 'G'; }
    } else {
      if (sameGroupG) { priority = 0; scope = 'G'; }
      else if (hasD) { priority = 1; scope = 'D'; }
    }

    if (priority >= 99) continue;
    candidates.push({ ex: ex, scope: scope, priority: priority, rand: Math.random() });
  }
  if (candidates.length === 0) return null;
  candidates.sort(function(a, b) {
    if (a.priority !== b.priority) return a.priority - b.priority;
    if (assignmentMode === 'NO_SAME_GROUP') return a.rand - b.rand;
    if (a.ex.currentLoad !== b.ex.currentLoad) return a.ex.currentLoad - b.ex.currentLoad;
    if (a.ex.maxLoad !== b.ex.maxLoad) return b.ex.maxLoad - a.ex.maxLoad;
    return String(a.ex.name || '').localeCompare(String(b.ex.name || ''));
  });
  candidates[0].totalCandidates = candidates.length;
  return candidates[0];
}

function findSelfSelectedExaminer_(selfExaminerName, badgeFullTitle, groupId, now) {
  var examinersList = _getActiveExaminersForAdmin_();
  for (var i = 0; i < examinersList.length; i++) {
    var ex = examinersList[i];
    if (ex.name !== selfExaminerName) continue;
    var hasD = ex.districtBadges.indexOf(badgeFullTitle) >= 0;
    var hasG = ex.groupBadges.indexOf(badgeFullTitle) >= 0;
    if (!hasD && !hasG) return { found: true, valid: false, error: '自行安排的主考無此章資格' };
    if (ex.currentLoad >= ex.maxLoad) return { found: true, valid: false, error: '自行安排的主考已滿載' };
    if (hasD) {
      return { found: true, valid: true, scope: 'D', examinerId: ex.examinerId, name: ex.name, email: ex.email, phone: ex.phone };
    }
    if (hasG && !isSameGroupUnit_(ex.unit, groupId)) {
      return { found: true, valid: false, error: '自行安排的 G 旅團主考不是考生所屬旅團' };
    }
    return { found: true, valid: true, scope: 'G', examinerId: ex.examinerId, name: ex.name, email: ex.email, phone: ex.phone };
  }
  return { found: false, valid: false };
}

function normalizeExaminerAssignmentMode_(mode) {
  mode = String(mode || '').trim().toUpperCase();
  if (mode === 'DISTRICT_PRIORITY') return 'DISTRICT_PRIORITY';
  if (mode === 'NO_SAME_GROUP') return 'NO_SAME_GROUP';
  return 'GROUP_PRIORITY';
}

function getExaminerAssignmentMode_() {
  return normalizeExaminerAssignmentMode_(getConfig('EXAMINER_ASSIGNMENT_MODE') || 'GROUP_PRIORITY');
}

function getExaminerAssignmentModeLabel_(mode) {
  mode = normalizeExaminerAssignmentMode_(mode || getExaminerAssignmentMode_());
  if (mode === 'DISTRICT_PRIORITY') return '只用區主考（公平模式）';
  if (mode === 'NO_SAME_GROUP') return '同旅團不能擔任主考';
  return '旅團主考優先';
}

function isSameGroupUnit_(examinerUnit, groupId) {
  var unit = String(examinerUnit || '').trim();
  if (!unit || !groupId) return false;
  var normalizedUnit = normalizeUnit_(unit);
  var info = getGroupFullData(groupId);
  if (!info) return false;
  var values = [groupId, info.groupName, info.groupNameCn, info.groupNumber ? ('第' + info.groupNumber + '旅') : ''];
  for (var i = 0; i < values.length; i++) {
    var v = String(values[i] || '').trim();
    if (!v) continue;
    var nv = normalizeUnit_(v);
    if (normalizedUnit === nv || normalizedUnit.indexOf(nv) >= 0 || nv.indexOf(normalizedUnit) >= 0) return true;
  }
  return false;
}
...
function setupSystem() {
  ...
  // ★ 確保 API_KEY_HASH 行存在（舊版 Sheet 升級時可能無呢行）
...
function apiGetHealthCheck() {
  try {
    ...
    return {
      ...
      assignmentMode: assignmentMode,
      assignmentModeLabel: getExaminerAssignmentModeLabel_(assignmentMode),
